(() => {
  ...
  const rawFlight = window.UAV_FLIGHT;
  ...
  function buildInstrumentedFlight(baseFlight) {
    if (!route) return baseFlight;
  ...
  const flight = buildInstrumentedFlight(rawFlight);
  window.UAV_FLIGHT = flight;
  ...
  function missionMetrics(execution, position, speed) {
    ...
    const crossTrack = position?.crossTrackNm == null ? null : Number(position.crossTrackNm);
